...
#define BDELAY 500                               //macro for the delay cycle function, can be easily changed
#define DELAY 500
...
void main(void)
{
    ...
    P1->SEL0 &= ~(BIT6);                            //setting up button
    P1->SEL1 &= ~(BIT6);
    P1->DIR &= ~(BIT6);
    P1->REN |= BIT6;
    P1->OUT |= BIT6;
    ...
    while(1)                                        //loops infinitely to run the program
   {
        ...
        case LED_OFF:                               //this if for the color black/off
            ...
            if(butPres2(b))                          //checks if the button has been pressed through our function
        ...
        case LED_YELLOW:
            ...
            P2->OUT |= (BIT1);                      //this turns on P2.2 to make color BLUE
            if(butPres(a))                          //checks if the button has been pressed through our function
                state =LED_RED;                     //if this is true it changes the state to RED
            if(butPres2(b))                          //checks if the button has been pressed through our function
                state =LED_GREEN;                     //if this is true it changes the state to GREEN
...
int butPres(uint8_t b)                              //This is our function to see if the button has been pressed
{
    if(!((P1->IN & BIT5)))                          //This action executes if button is pressed
    {
        ...
        while(!((P1->IN & BIT5))){__delay_ms(DELAY);break;};                //This is for the button when it its being continually pressed
...
int butPres2(uint8_t b)                              //This is our function to see if the button has been pressed
{
    if(!((P1->IN & BIT6)))                          //This action executes if button is pressed
    {
        __delay_ms(BDELAY);                          //This is our debouncer, its immediately after  the button being pressed
        while(!((P1->IN & BIT6))){__delay_ms(DELAY);break;};                //This is for the button when it its being continually pressed
        b = 1;                                      //This is to return a value after the button is released
    }
    else                                            //This is if the button was not pressed
    {
