...
#define DELAY 3                               //macro for the delay cycle function, can be easily changed
    ...
    while(1)                                        //loops infinitely to run the program
   {
        ...
        case LED_OFF:                               //this if for the color black/off
            ...
            if(butPres(a))                          //checks if the button has been pressed through our function
                state =LED_GREEN;                     //if this is true it changes the state to RED
        ...
        case LED_GREEN:
            ...
            if(butPres(a))                          //checks if the button has been pressed through our function
                state =LED_YELLOW;                    //if this is true it changes the state to BLUE
            break;                                  //breaks out of the switch statement and continues in the while
        case LED_YELLOW:
            P2->OUT &= ~(BIT0|BIT2);                //this turns pin 0 and 1 off on port 2
            P2->OUT |= (BIT1);                      //this turns on P2.2 to make color BLUE
            if(butPres(a))                          //checks if the button has been pressed through our function
                state =LED_RED;                     //if this is true it changes the state to BLACK/OFF
            break;                                  //breaks out of the switch statement and continues in the while
        case LED_RED:
...
int butPres(uint8_t b)                              //This is our function to see if the button has been pressed
{
    ...
    {
        __delay_ms(DELAY);                      //This is our debouncer, its immediately after  the button being pressed
