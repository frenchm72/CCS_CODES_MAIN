...
uint16_t keyS = 0x0000;                     //declares some global variables
char key[5], key1[5], new[1], pin[5];
int i, a, morKey;

void main(void)
{
    WDT_A->CTL = WDT_A_CTL_PW | WDT_A_CTL_HOLD;//stops watchdog timer

    pin[0] = '1';                           //to declare the pin that allows access .
    pin[1] = '2';                           //you can change the 4 digit pin here as well
    pin[2] = '3';
    pin[3] = '4';
    pin[4] = '\0';
...
while(1)                                    //this is a while loop so if the user gets it wrong or entered wrong it'll ask you to keep trying
{
    if(morKey==1)
    {
        getKey();
...
if(morKey == 0)//this is to make sure you dont need more keys
{
    ...
    {
        if(!(strcmp(key, pin)))            //this checks if the pin is the aloud one
        {
            printf("\n!!!!!!Access Granted!!!!!\n");
            break;
        }
        else                               //this is if the pin was entered but the pin wasn't the right one
        {
            printf("\n!!Wrong Key !!!\n Try again\n");
            restKey();
            getKey();
