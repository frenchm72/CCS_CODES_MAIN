...
#define DELAY 3000000                               //macro for the delay cycle function, can be easily changed
...
void main(void)
{
    ...
    P1->SEL0 &= ~(BIT1);                            //setting up button
    P1->SEL1 &= ~(BIT1);
    P1->DIR |= ~(0xFF);
    P1->REN |= BIT1;
    P1->OUT |= BIT1;
    ...
    while(1)                                        //loops infinitely to run the program
   {
        ...
        case LED_RED:
            ...
            break;                                  //breaks out of the switch statement and continues in the while
        case LED_GREEN:
            P2->OUT &= ~(BIT0|BIT2);                //this turns pin 0 and 2 off on port 2
            P2->OUT |= (BIT1);                      //this turns on P2.1 to make color GREEN
            if(butPres(a))                          //checks if the button has been pressed through our function
                state =LED_BLUE;                    //if this is true it changes the state to BLUE
            break;                                  //breaks out of the switch statement and continues in the while
        case LED_BLUE:
            P2->OUT &= ~(BIT0|BIT1);                //this turns pin 0 and 1 off on port 2
            P2->OUT |= (BIT2);                      //this turns on P2.2 to make color BLUE
            if(butPres(a))                          //checks if the button has been pressed through our function
                state =LED_OFF;                     //if this is true it changes the state to BLACK/OFF
            break;                                  //breaks out of the switch statement and continues in the while
        }
    }
...
{
    if(!((P1->IN & BIT1)))                          //This action executes if button is pressed
    {
        __delay_cycles(DELAY);                      //This is our debouncer, its immediately after  the button being pressed
